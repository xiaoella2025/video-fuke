...
(async function () {
  ...
  cl.courses.forEach(c => {
    ...
    if (c.previewVideo) o.dataset.preview = c.previewVideo;
    sel.appendChild(o);
  });
  ...
  function renderCasePreview() {
    ...
    v.addEventListener("error", () => {
      ...
      ph.innerHTML = `案例视频待上传<br><small style="opacity:.7">请将 mp4 放到：${esc(vidPath)}</small>`;
      wrap.appendChild(ph);
    });
    wrap.appendChild(v);
    el.appendChild(wrap);
  ...
  function renderChain(ch, idx) {
    ...
    // 'relations' cells are metadata (downstream linkages), not for student view
    const track = document.createElement("div"); track.className = "chain-track";
    const visibleCells = (ch.cells || []).filter(c => c && c.type !== "relations");
  ...
  function renderCell(cell) {
    if (cell.type === "inputs") {
      ...
      if (!imgs.length && !vids.length) {
        ...
        vids.forEach(p => c.appendChild(makeAssetThumb(p, true, "输入视频")));
      }
      return c;
    }
    if (cell.type === "prompt") {
